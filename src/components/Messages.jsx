...
const Message = ({ message }) => {
  ...
  return (
    <div>
      <div className={`flex w-fit items-center shadow-xl my-10 py-2 px-3 rounded-tl-full rounded-tr-full ${messageClass(message.uid)}`}> 
        <div className='flex gap-2 absolute mt-[-4.5rem] '>
            <p className=" dark:text-white text-xs">{message.name}</p>
            <img className='w-6 h-6 rounded-full' src={message.photoURL} alt="" />
        </div>
